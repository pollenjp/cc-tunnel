...
| ポート | `-p {dynamic_port}:9091`（ホスト側ポートは既定 61000-61999 の範囲で動的割り当て。IANA Dynamic/Private のうち Linux ephemeral と被らない帯。`PortRangeStart` / `PortRangeEnd` で変更可能） |
...
| コンポーネント | フィールド / 環境変数 | デフォルト | 説明 |
|---|---|---|---|
| cc-tunnel | `ZeroAgentsTimeout` / `GCE_VM_ZERO_AGENTS_TIMEOUT_SECONDS` | `600` (10 分) | `ReconcileVMs` が VM を削除するしきい値 |
| cc-tunnel | `VMReconcileInterval` / `GCE_VM_RECONCILE_INTERVAL_SECONDS` | `0` (無効) | in-process ticker。**Cloud Run 上では 0 にしておく**。local dev のみ設定 |
| cc-tunnel | `ContainerManagerProbeTimeout` / `GCE_CONTAINER_MANAGER_PROBE_TIMEOUT_SECONDS` | `5` 秒 | VM ごとの `GET /v1/agents` タイムアウト |
| container-manager | `SELF_REAP_TIMEOUT_SECONDS` | `600` (10 分) | 自己 agent count ゼロ継続で VM を自己削除するしきい値 |
| container-manager | `SELF_REAP_INTERVAL_SECONDS` | `60` | 自己 reap loop の周期 |
| Cloud Scheduler | cron | `0 */6 * * *` | 6 時間ごとに `/internal/reconcile-vms` を発火 |
