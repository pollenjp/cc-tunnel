...
└──────┬──────────────────────────────┬──────────────┬────────────┘
       │ /var/run/docker.sock (DooD)  │ HTTP :9091   │ pgx/v5
       ▼                              ▼              ▼
┌────────────────┐  ┌──────────────────────────┐  ┌─────────────────────┐
│ Docker daemon  │  │  cc-remote-agent-auth    │  │  PostgreSQL         │
│ (ホスト共有)   │  │  認証専用常駐コンテナ    │  │  会話・メッセージ   │
└───────┬────────┘  │  claude /auth PTY 管理   │  │  永続化             │
        │ manage    └──────────────────────────┘  └─────────────────────┘
        ▼
┌─────────────────────────────────────────────────────┐
│  cctunnel-session-{convID[:8]} (per-session)        │
│  cc-remote-agent コンテナ（会話ごとに動的生成）     │
│  Volume: claude-sessions → /home/user/.claude       │
...
3. ユーザーが「ログイン」ボタン押下
4. POST /api/auth/login → cc-tunnel → cc-remote-agent-auth POST /auth/login
5. cc-remote-agent-auth: AuthManager.StartLogin()
...
7. cc-remote-agent-auth: outputBuf の since バイト以降を base64 エンコードして返す
...
11. cc-remote-agent-auth: AuthManager.SubmitInput() → PTY stdin に書き込み
...
| コンポーネント | 説明 |
|--------------|------|
| `cc-remote-agent-auth` | 認証専用常駐コンテナ。`-agent-url` で指定。claude /auth の PTY 管理のみを担当 |
...
| ボリューム | `claude-sessions` → `/home/user/.claude` |
...
`docker_gce` provider は別途 GCE VM 上の Docker Host でコンテナを管理する将来実装（現在 mock）。
...
| HostMode | **廃止済み**（`CC_TUNNEL_HOST_MODE` 環境変数も削除）|

### HostMode 廃止について

旧来の `CC_TUNNEL_HOST_MODE` によるホストネットワーク直接アクセス方式は廃止された。
DooD + Docker ネットワーク DNS 名解決に統一されている。
...
| `apps/compose.yaml` | **サービス起動**: postgres + cc-remote-agent-auth（デフォルト）、profile "full" で全サービス起動 |
...
| `cc-remote-agent-auth` | 認証専用常駐コンテナ（`cc-remote-agent:latest`）、`claude-sessions` ボリュームマウント |
...
# 2. 基本サービス起動（postgres + cc-remote-agent-auth）
...
| 値 | Provider | 説明 |
|----|----------|------|
| `local` | local.LocalDockerProvider | DooD 方式で per-session Docker コンテナを動的生成（agentURL = cc-remote-agent-auth の URL） |
...
```
EXECUTION_PROVIDER=local cc-tunnel -agent-url http://cc-remote-agent-auth:9091
```
