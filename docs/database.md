...
**metadata の主なフィールド**（assistant メッセージ）:
...
**後方互換**:
- `content_blocks` が存在しない旧メッセージは正常に読み込める（`nil` チェック推奨）
- `tool_calls` の旧フォーマットでは `name` / `input` フィールドを使用していた。現行フォーマットは `tool_name` / `input_json` に統一済み。旧メッセージ参照時は両フィールド名でのフォールバックが必要
